# entrypoints/cli/ingest_properties_rentcast_parallel.py
import argparse
import time
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Parallel ingest of sale listings from RentCast"
    )
    parser.add_argument(
        "--zip",
        dest="zips",
    ...
    parser.add_argument("--max-price", dest="max_price", type=float, default=None)
    parser.add_argument("--limit", dest="limit", type=int, default=300)
    parser.add_argument(
        "--workers",
        dest="workers",
        type=int,
...
def main() -> None:
    ...
    print(
        ...
        f"with {args.workers} workers (RentCast)..."
    )
